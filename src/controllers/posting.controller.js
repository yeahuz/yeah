...
export async function update_attachment(req, reply) {
  ...
  switch (_action) {
    case "delete_attachment": {
      ...
      const uploaded_files = existing.uploaded_files?.filter(
        (f) => f.id !== attachment_id
      );

      await redis_client.set(
        id,
        JSON.stringify(Object.assign(existing, { uploaded_files }))
      );
    }
    default:
      break;
...
export async function submit_first_step(req, reply) {
  ...
  await redis_client.set(
    id,
    JSON.stringify(Object.assign(existing, { title, category_id }))
  );
...
export async function submit_second_step(req, reply) {
  ...
  await redis_client.set(
    id,
    JSON.stringify(
      Object.assign(existing, Object.assign(body, { uploaded_files }))
    )
  );
...
export async function submit_third_step(req, reply) {
  ...
  const [formatted_address, lat, lon, district_id, region_id] =
    location.split("|");
  const existing = JSON.parse((await redis_client.get(id)) || null) || {};
  await redis_client.set(
    id,
...
export async function get_step(req, reply) {
  ...
  switch (step) {
    case "1": {
      const posting_data =
        JSON.parse((await redis_client.get(id)) || null) || {};
    ...
    case "2": {
      const posting_data =
        JSON.parse((await redis_client.get(id)) || null) || {};
      const valid = req.validateInput(
        posting_data,
        new_posting_schema.essential
      );
      if (!valid) {
