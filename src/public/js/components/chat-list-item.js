...
export class ChatListItemClass {
  constructor(data) {
    this.data = data;
    this.el = li(
      {
...
export function ChatListItem(chat) {
  let component = li({
      class: "flex flex-col hover:bg-gray-100 dark:hover:bg-zinc-800 duration-200 p-4 relative",
      id: add_prefix("chat", chat(c => c.id))
    },
    a({
      href: `/chats/${chat(c => c.id)}`,
      class: "after:absolute after:top-0 after:left-0 after:right-0 after:bottom-0"
    }),
    div(
      ...
      div(
        ...
        img({
          src: chat(c => c.posting.cover_url) + "/width=80",
          class: "w-12 h-12 object-cover rounded-full overflow-hidden mr-2 flex-shrink-0",
          crossorigin: "anonymous"
        }),
        div(
          { class: "flex flex-col max-w-[200px]" },
          span({ class: "text-gray-500 dark:text-gray-300 text-sm truncate" }, chat(c => c.posting.creator)),
          a(
            { href: chat(c => c.posting.url), class: "relative underline decoration-transparent hover:decoration-white duration-200 text-gray-700 dark:text-gray-200 font-medium truncate" },
            chat(c => c.posting.title)
          )
        ),
      ),
      div(
