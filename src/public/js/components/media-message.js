import { li, ul, div, classes, html, span, form, input, condition, button, img, text } from "dom";
...
function Attachments(message) {
  ...
  for (let a of message.files?.() || message.attachments) {
    ...
    let item = li(
      ...
      message.is_own() && span(
        { class: () => `absolute top-0 left-0 w-full h-full bg-black/70 items-center justify-center rounded-lg ${a.uploading() ? "flex" : "hidden"}` },
        text(() => `${a.progress()}%`)
      )
    );
